...
function getDayName(date) {
  ...
  return arrDays[new Date(date).getDay()];
}
...
function getNextFriday(date) {
  ...
  if (startDay.getDay() < 5) {
    const NumDayToFriday = 5 - startDay.getDay();
    nextFriday.setDate(startDay.getDate() + NumDayToFriday);
  } else if (startDay.getDay() === 5) {
    nextFriday.setDate(startDay.getDate() + 7);
  } else {
    nextFriday.setDate(startDay.getDate() + 6);
  }
  return nextFriday;
}
...
function isDateInPeriod(date, period) {
  ...
  return tmpDate >= start && tmpDate <= end;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= countDayInMonth; i += 1) {
    const tmpDayInMonth = new Date(year, month - 1, i).getDay();
    if (tmpDayInMonth === 6 || tmpDayInMonth === 0) {
      countHolidays += 1;
    }
  }
...
function getNextFridayThe13th(date) {
  ...
  while (tmpDate.getDate() !== 13 || tmpDate.getDay() !== 5) {
